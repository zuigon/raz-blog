...
module Toto
  ...
  class Site
    ...
    def all type = :html
      articles = type == :html ? self.articles.reverse : self.articles
      {:articles => articles.collect do |article| Article.new article, @config end}
    end

    def tags name, type = :html
      articles = type == :html ? self.articles.reverse : self.articles
      {:articles => articles.collect do |article|
        x = Article.new article, @config
        if x[:tags] and x[:tags].include? name
          x
        end
      end.select{ |x| !x.nil?  }
      }
    ...
    def go route, type = :html, env = {}
      ...
      if @config[:sites]
        if x = @config[:sites][env["SERVER_NAME"]]
          @config[:blk_hosts], @config[:real_blog_hosts], @config[:fake_page] = x[:blk_hosts], x[:real_blog_hosts], x[:fake_page]
        elsif x = @config[:sites]["default"]
          @config[:blk_hosts], @config[:real_blog_hosts], @config[:fake_page] = x[:blk_hosts], x[:real_blog_hosts], x[:fake_page]
        end
      end

      body, status = if @config[:blk_hosts].include? env["REMOTE_ADDR"] or @config[:blk_hosts][0] == "all"
          puts "[logger] blocked access from: #{env["REMOTE_ADDR"]}"
          [nil, 400]
        elsif @config[:real_blog_hosts].include? env["REMOTE_ADDR"] or
          @config[:real_blog_hosts][0] == "all" or ENV['RACK_ENV'] == 'test' or !@config[:real_blog_hosts]
          puts "[logger] real blog access from: #{env["REMOTE_ADDR"]}"
          if Context.new.respond_to?(:"to_#{type}")
            if route.first == 'tags'
              if route[1] and !route[2]
                context[tags(route[1], type), :tags]
              end
            elsif route.first == 'all'
              context[all(type), :all]
            elsif route.first =~ /\d{4}/ and (1900..2100).include? route.first.to_i
              case route.size
              when 1..3
          ...
          puts "[logger] fake page to: #{env["REMOTE_ADDR"]}"
          [@config[:fake_page].call, 200]
        end
